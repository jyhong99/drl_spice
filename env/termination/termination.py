...
class TerminationModel:
    ...
    def check(self, *, step: int, non_convergent: bool) -> Tuple[bool, DoneReason]:
        ...
        Notes
        -----
        Time-limit termination has priority over non-convergence only in
        evaluation order; semantically, both are terminal conditions.
        """
        if int(step) >= int(self.max_steps):
            return True, DONE_TIME_LIMIT  # type: ignore[return-value]

        if bool(non_convergent):
            return True, DONE_NON_CONV  # type: ignore[return-value]

        return False, DONE_RUNNING  # type: ignore[return-value]
...
class ResetRule:
    ...
    def decide(self, *, done_reason: DoneReason) -> ResetMode:
        ...
        - Otherwise                -> ``RESET_CONTINUE``
        """
        if done_reason == DONE_NON_CONV:
            return RESET_RANDOM  # type: ignore[return-value]

        if done_reason == DONE_TIME_LIMIT:
        ...
        return RESET_CONTINUE  # type: ignore[return-value]
