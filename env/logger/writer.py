...
class CSVWriter:
    ...
    def write_event(self, event: Dict[str, Any]) -> None:
        ...
        # Caller may directly provide payload_json; accept string/JSON-like objects.
        if PAYLOAD_JSON_KEY in event:
            payload_val = event[PAYLOAD_JSON_KEY]
            if isinstance(payload_val, str):
                row[PAYLOAD_JSON_KEY] = payload_val
            else:
                row[PAYLOAD_JSON_KEY] = _json_cell(payload_val)
        ...
        if payload:
            if row[PAYLOAD_JSON_KEY]:
                try:
                    base = json.loads(row[PAYLOAD_JSON_KEY])
                    if not isinstance(base, dict):
                        base = {"_base_payload": base}
                except Exception:
                    base = {"_base_payload_raw": row[PAYLOAD_JSON_KEY]}
                base["_extra"] = _to_jsonable(payload)
                row[PAYLOAD_JSON_KEY] = json.dumps(base, ensure_ascii=False)
            else:
        ...
        self.write_event(
            {
                ...
                PAYLOAD_JSON_KEY: _to_jsonable(env_config),
            }
        )
