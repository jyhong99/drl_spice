...
def _require_x_in_bounds(
    ...
    *,
    ...
    if (x_min < float(clip_low) - float(x_tol)) or (x_max > float(clip_high) + float(x_tol)):
        raise ValueError(
...
def _clip_or_validate_action(
    ...
    *,
    ...
    """
    if clip_action:
        return np.clip(a, float(action_low), float(action_high))
    ...
    tol = float(eps)
    if (a_min < low - tol) or (a_max > high + tol):
        raise ValueError(
            f"action out of bounds [{low},{high}]: min={a_min}, max={a_max}"
        )
    return a
...
def _snap_step_scalar(v: float, *, step: float, p_min: float, p_max: float) -> float:
    ...
    return float(np.clip(snapped, float(p_min), float(p_max)))
...
def _interp_linear(t: float, *, p_min: float, p_max: float) -> float:
    ...
    Notes
    -----
    This function does not clamp ``t``. Call :func:`_clip01_scalar` upstream
    when strict interpolation in ``[0, 1]`` is required.
    """
    t = float(t)
    p_min = float(p_min)
    p_max = float(p_max)
...
def _interp_log10(t: float, *, p_min: float, p_max: float) -> float:
    ...
    if p_min <= 0.0 or p_max <= 0.0:
        raise ValueError(f"log10 interpolation requires positive bounds, got {p_min}, {p_max}")
