...
def _as_1d_f64(x: Any) -> np.ndarray:
    ...
    ------
    TypeError
        If ``x`` cannot be interpreted as an array-like object by NumPy.
...
def _safe_denom(spec: np.ndarray, ref: np.ndarray, *, eps_denom: float) -> np.ndarray:
    ...
    eps_denom = float(eps_denom)
    if eps_denom < 0.0:
        raise ValueError(f"eps_denom must be >= 0, got {eps_denom}")
    denom = spec - ref
...
def _is_non_convergent(o: np.ndarray) -> bool:
    ...
    Notes
    -----
    ``o`` is expected to be numeric. Non-numeric object arrays may propagate
    NumPy type errors from ``np.isfinite``.
    """
...
def _u_vec(*, non_convergent: bool, unstable: bool) -> np.ndarray:
    ...
    if U_DIM > 0:
        u[0] = float(bool(non_convergent))
    if U_DIM > 1:
        u[1] = float(bool(unstable))
    return u
