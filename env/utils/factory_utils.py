...
def _prepare_spice_scripts_env(
    ...
    *,
    ...
    stable environment variable (``SPICE_SCRIPTS``).
    ...
    -------
    KernelEnv
        Environment mapping containing `{\"SPICE_SCRIPTS\": <dir>}` or an empty
    ...
    ``<repo_root-or-home>/ngspice/ngspice_scripts``.
    """
...
def _resolve_templates(
    ...
    *,
    ...
    merged: Dict[str, Path] = {k: _resolve_path(v) for k, v in defaults.items()}
    for k, v in (netlist_templates or {}).items():
