...
def _redirect_write_paths(netlist_text: str, run_dir: PathLike) -> str:
    ...
    Typical use-case:
    - Templates may use relative paths (or fixed paths) for `.wrdata` outputs.
    - For reproducibility and isolation, each simulation run should write outputs
    ...
    Notes
    -----
    - Only the *basename* of the original output path is kept:
      `Path(path_tok).name`.
      This prevents path traversal and keeps filenames stable.
    - Command name is lowercased to normalize output.
    - Lines not matching `_WRITE_RE` are preserved verbatim.
    """
    rd = Path(run_dir)
    out: list[str] = []
...
def _ensure_noise_writes_frequency(netlist_text: str) -> str:
    ...
    Some ngspice workflows produce a "bare" write/wrdata command for noise results,
    which may omit frequency. This helper patches such lines to include:

        frequency NoiseFigure
    ...
    Notes
    -----
    - Matching is performed using `_NOISE_WRITE_BARE_RE` on `ln.strip()`.
    - If a line matches, it is rewritten as:
        `{cmd} {path} frequency NoiseFigure`
      where `cmd` is normalized to lowercase.
...
def _set_sky130_lib(netlist_text: str, *, lib_path: PathLike, corner: str = "tt") -> str:
    ...
    This function scans the template netlist for a `.lib` line that references
    `sky130.lib.spice` and replaces that line with:
    ...
    corner : str, default="tt"
        Process corner string (e.g., "tt", "ff", "ss"). The string is stripped.
    ...
    ------
    ValueError
        If no `.lib` line containing "sky130.lib.spice" is found. This enforces
        a template contract: each template must contain a placeholder `.lib`
        line that can be replaced programmatically.

    Notes
    -----
    - The function only replaces `.lib` lines whose captured path contains the
      substring "sky130.lib.spice".
    - If the template contains `.lib` lines unrelated to sky130, they are left
      unchanged.
    """
...
def _coerce_float_knob(x: KnobValue) -> Optional[float]:
    ...
    Notes
    -----
    - Integers and floats are converted directly.
    - Strings are stripped then parsed via `float(...)`.
    - Any parsing error returns None.
    """
    try:
...
def _sorted_str_keys(d: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    Notes
    -----
    Deterministic key order is useful when emitting parameter blocks or
    logging configs to ensure stable diffs and reproducible artifacts.
    """
...
def _normalize_knobs(knobs: Mapping[str, KnobValue]) -> Dict[str, KnobValue]:
    ...
    Currently normalizes the Q-factor knob name by ensuring both variants exist:
    - "Q_factor"
    - "q_factor"
    ...
    Notes
    -----
    - If only one of ("Q_factor", "q_factor") is present, the other is added
      with the same value.
    - If both are present, values are left unchanged (no conflict resolution).
    """
