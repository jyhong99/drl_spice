...
class RewardModel:
    ...
    def compute(
        ...
        *,
        ...
        Non-finite incoming costs are replaced with `bad_cost` to keep reward
        outputs finite and training numerically stable.
        """
        J_viol_f = float(J_viol)
        J_fom_f = float(J_fom)
        var_f_f = float(var_f)
        bad_cost = float(self.bad_cost)
        ...
        tJ_viol = self._penalize(J_viol_f, invalid=bool(invalid), kappa=float(self.kappa_viol))
        tJ_fom = self._penalize(J_fom_f, invalid=bool(invalid), kappa=float(self.kappa_fom))
