...
class ConstraintModel:
    ...
    _spec_vec: np.ndarray = field(init=False, repr=False)
    _ref_vec: np.ndarray = field(init=False, repr=False)
    _w: np.ndarray = field(init=False, repr=False)
    _denom_vec: np.ndarray = field(init=False, repr=False)
    _p: float = field(init=False, repr=False)
    _inv_p: float = field(init=False, repr=False)
    _eps_viol0_f: float = field(init=False, repr=False)
    _eps_wsum_f: float = field(init=False, repr=False)
    _use_k: bool = field(init=False, repr=False)
    _k_min_f: float = field(init=False, repr=False)
    ...
    def __post_init__(self) -> None:
        ...
        if self.w is None:
            ...
            if float(np.sum(w)) <= 0.0:
                raise ValueError("w must contain at least one positive entry")
            self.w = w
        ...
        spec_vec = np.array([float(spec_dict[k]["value"]) for k in self.perf_keys], dtype=np.float64)
        ref_vec = np.array([float(self.perf_spec.references[k]) for k in self.perf_keys], dtype=np.float64)
        for k, spec_v, ref_v in zip(self.perf_keys, spec_vec, ref_vec):
            _require_spec_ref_separation(
                ctx=str(ctx),
                key=str(k),
                spec_v=spec_v,
        ...
        self._spec_vec = spec_vec
        self._ref_vec = ref_vec
        self._w = self.w
        self._denom_vec = _safe_denom(spec_vec, ref_vec, eps_denom=float(self.eps_denom))
        self._p = float(self.p)
        self._inv_p = 1.0 / self._p
        self._eps_viol0_f = float(self.eps_viol0)
        self._eps_wsum_f = float(self.eps_wsum)
        self._use_k = bool(self.use_k)
        self._k_min_f = float(self.k_min)

    def _to_metrics_dict(
        self,
        performances: np.ndarray,
        aux: Optional[Mapping[str, float]],
    ) -> Dict[str, float]:
        """
        ...
        ----------
        performances : np.ndarray
            One-dimensional performance vector aligned to `self.perf_keys`.
            The expected length is ``len(self.perf_keys)``.
        ...
        """
        d: Dict[str, float] = {k: float(v) for k, v in zip(self.perf_keys, performances)}
        if aux is not None and not isinstance(aux, Mapping):
            raise ValueError(f"aux must be a mapping or None, got {type(aux).__name__}")
    ...
    def compute(
        ...
        *,
        ...
        ----------
        performances : np.ndarray
            One-dimensional performance vector aligned with ``self.perf_keys``.
            Shape must be ``(n_metrics,)`` where ``n_metrics = len(self.perf_keys)``.
            Values may include non-finite entries; those entries are replaced by
            reference values for constraint arithmetic to preserve finite outputs.
        aux : dict[str, float] or None, default=None
            Extra scalar metrics (e.g., K-factor minima, bandwidth, or
            simulator-side diagnostics). Every entry is converted to ``float`` and
            merged into the returned metrics dictionary.
        ...
        dict[str, Any]
            ...
            - ``F_KEY`` : np.ndarray, shape ``(n_metrics,)``
              Normalized satisfaction vector ``f = (o - ref) / denom``.
            - ``VIOL_KEY`` : np.ndarray, shape ``(n_metrics,)``
              Per-metric violation vector ``viol = max(0, 1 - f)``.
            - ``VIOL_TERM_KEY`` : float
              Weighted p-aggregation:
              ``((sum(w * viol**p)) / (sum(w) + eps_wsum))**(1/p)``.
            - ``SAT_KEY`` : bool
              ``True`` when every element in ``viol`` is ``<= eps_viol0``.
            - ``FEASIBLE_KEY`` : bool
              ``True`` when ``SAT_KEY`` is true and invalidity flags are clear.
            - ``U_KEY`` : np.ndarray, shape ``(2,)``
              Invalidity indicator vector ``[non_convergent, unstable]``.
            - ``NONCONV_KEY`` : bool
              ``True`` when non-convergence is detected.
            - ``UNSTABLE_KEY`` : bool
              ``True`` when instability is detected (currently via ``K_MIN_KEY``).
            - ``INVALID_KEY`` : bool
              ``True`` when either invalidity indicator is active.
            - ``METRICS_KEY`` : dict[str, float]
              Named scalar metrics map containing performance metrics and ``aux``.
            - ``SPEC_KEY`` : np.ndarray, shape ``(n_metrics,)``
              Spec values aligned with ``PERF_KEYS_KEY``.
            - ``REF_KEY`` : np.ndarray, shape ``(n_metrics,)``
              Reference values aligned with ``PERF_KEYS_KEY``.
            - ``PERF_KEYS_KEY`` : tuple[str, ...]
              Ordered metric key tuple used by all returned vectors.
            - ``ENABLE_LIN_KEY`` : bool
              Whether linearity-related metrics were enabled.

        Raises
        ------
        ValueError
            If ``performances`` length does not match ``len(self.perf_keys)``,
            or ``aux`` is not a mapping (when provided).

        Notes
        -----
        Constraint normalization uses both spec and reference values and an
        epsilon-protected denominator prepared at initialization time. This keeps
        per-step computation concise and stable while preserving the original
        reward semantics.
        """
        o = _as_1d_f64(performances)
        _require_len("performances", o, len(self.perf_keys))
        metrics = self._to_metrics_dict(o, aux)
        ...
        non_convergent = bool(non_convergent)

        # Keep constraint vectors finite even if performances contain NaN/Inf.
        if not np.all(np.isfinite(o)):
            o = np.where(np.isfinite(o), o, ref)

        f = (o - ref) / self._denom_vec
        viol = np.maximum(0.0, 1.0 - f)

        wsum = float(np.sum(self._w)) + self._eps_wsum_f
        viol_term = float((np.dot(self._w, viol ** self._p) / wsum) ** self._inv_p)

        satisfied_perf = bool(np.all(viol <= self._eps_viol0_f))

        unstable = False
        if self._use_k and (K_MIN_KEY in metrics):
            k_val = float(metrics[K_MIN_KEY])
            unstable = (not np.isfinite(k_val)) or bool(k_val < self._k_min_f)

        u = _u_vec(non_convergent=non_convergent, unstable=bool(unstable))
...
class ObjectiveModel:
    ...
    *negative* FoM (i.e., minimize J = -FoM).
    ...
    def __post_init__(self) -> None:
        """
        Validate numeric stability hyperparameter.
        ...
        """
        self._eps = _require_finite_positive("eps", float(self.eps))
    ...
    def fom_db(self, performances: np.ndarray) -> float:
        ...
        ----------
        performances : np.ndarray
            One-dimensional performance vector in the metric order expected by
            the environment. The first ``MIN_OBJ_LEN`` entries must map to:
            ``[S11_dB, S21_dB, S22_dB, NF_dB, PD_mW]``.
        ...
        float
            ...
            ``DEFAULT_BAD_FOM`` is returned when:
            - fewer than ``MIN_OBJ_LEN`` values are available, or
            - one or more required base entries is non-finite.

        Raises
        ------
        None
            This method is intentionally non-raising for malformed numeric
            inputs and instead returns ``DEFAULT_BAD_FOM``.
        ...
        o = _as_1d_f64(performances)
        vals = o[:MIN_OBJ_LEN]
        if vals.shape[0] < MIN_OBJ_LEN:
            return float(DEFAULT_BAD_FOM)
        if not np.all(np.isfinite(vals)):
            return float(DEFAULT_BAD_FOM)
        s11_db, s21_db, s22_db, nf_db, pd_mw = vals

        # Practical clipping for stability
        pd_mw = max(pd_mw, 1e-9)
        nf_db = min(max(nf_db, 0.0), 200.0)
        ...
        one_minus_in = max(1.0 - s11, self._eps)
        one_minus_out = max(1.0 - s22, self._eps)
        ...
        fom = float(
            ...
            - 10.0 * np.log10(noisefactor + self._eps)
            - 10.0 * np.log10(pd_mw + self._eps)
        )

        # Optional: add linearity bonus (e.g., IIP3)
        if o.shape[0] > IIP3_INDEX:
            iip3_dbm = o[IIP3_INDEX]
