...
class SpecDecoder:
    ...
    def decode_x_to_physical(self, x: Sequence[float]) -> np.ndarray:
        ...
        ------
        ValueError
            If `x` does not have the expected length, contains non-finite values,
            or violates [0, max_param] when `clip_x=False`.
        ...
        if self.clip_x:
            ...
            if x_min < 0.0 or x_max > float(self.max_param):
                raise ValueError(
                    f"x out of bounds [0, {float(self.max_param)}] with clip_x=False: "
