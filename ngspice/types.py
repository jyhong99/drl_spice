...
class AnalysisKnobSet:
    ...
    def defaults(self) -> Dict[str, object]:
        ...
        -------
        dict[str, object]
            Mapping from knob name to default value in declaration order.

        Notes
        -----
        Keys are unique by construction of ``specs``; later duplicates would
        overwrite earlier ones if present.
        """
...
class ParamKeys:
    ...
    def __len__(self) -> int:
        """
        Return number of parameter keys.
    ...
    def as_tuple(self) -> Tuple[str, ...]:
        """
        Return keys as an immutable tuple.

        Returns
        -------
        tuple[str, ...]
            Parameter-key tuple preserving declared order.
        """
        return self.keys
...
class ParamDomain:
    ...
    def __post_init__(self) -> None:
        """
        Validate key/spec shape and ordering consistency.

        Raises
        ------
        ValueError
            If key/spec lengths differ or if names/order mismatch.
        """
