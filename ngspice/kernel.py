...
class Kernel:
    ...
    def __init__(
        ...
        *,
        ...
        self.ngspice_bin = str(ngspice_bin)
    ...
    def _write_logs(
        ...
        Notes
        -----
        - Uses default encoding of `Path.write_text` (platform-dependent). If you
          want strict UTF-8, pass `encoding="utf-8"` explicitly.
        """
        (run_cwd / stdout_name).write_text(stdout or "")
        (run_cwd / stderr_name).write_text(stderr or "")
    ...
    def run(
        ...
        *,
        ...
        run_cwd = self._ensure_dir(Path(cwd) if cwd is not None else netlist_path.parent)
