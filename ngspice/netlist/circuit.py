...
class Circuit:
    ...
    def __init__(self, netlist_path: PathLike, *, names: CircuitNames) -> None:
        """
        Initialize a parsed-circuit view from a rendered netlist.

        Parameters
        ----------
        netlist_path : PathLike
            Filesystem path to the rendered netlist text.
        names : CircuitNames
            Canonical naming contract containing expected design-variable names
            and device instance names.

        Raises
        ------
        NetlistFormatError
            If device or variable names contain case-insensitive duplicates.
        CircuitNetlistNotFound
            If ``netlist_path`` does not exist.
        NetlistMarkerError
            If required marker blocks are missing.
        """
        ...
        self._dev_lookup = {d.lower(): d for d in self.device_names}
        self._var_lookup = {v.lower(): v for v in self.designvar_names}
        if len(self._dev_lookup) != len(self.device_names):
            raise NetlistFormatError("device_names contains duplicates under case-insensitive matching")
        if len(self._var_lookup) != len(self.designvar_names):
            raise NetlistFormatError("designvar_names contains duplicates under case-insensitive matching")
        self._dev_set = set(self._dev_lookup)
        self._var_set = set(self._var_lookup)
    ...
    def _map_dsgnvar_to_val(self) -> Dict[str, float]:
        ...
        for m in _RE_PARAM.finditer(block):
            name = m.group(1)
            lname = name.lower()
            if lname not in self._var_set:
                continue
            out[self._var_lookup[lname]] = float(m.group(2))
        return out
    ...
    def _map_device_to_dsgnvar(self) -> Dict[str, str]:
        ...
        for raw_line in block.splitlines():
            ...
            if inst_l not in self._dev_set:
                continue
            inst_canonical = self._dev_lookup[inst_l]
            ...
            for key, var in _RE_ASSIGN_BRACED.findall(line):
                lvar = var.lower()
                if lvar in self._var_set:
                    dvc_to_dsgnvar[f"{inst_canonical}_{key}"] = self._var_lookup[lvar]
                    found_any = True
            ...
            if not found_any:
                for var in _RE_BRACED_VAR.findall(line):
                    lvar = var.lower()
                    if lvar in self._var_set:
                        dvc_to_dsgnvar[inst_canonical] = self._var_lookup[lvar]
                        found_any = True
                        break
            ...
            if not found_any:
                for tok in line.split():
                    tok_norm = tok.strip("{}(),")
                    ltok = tok_norm.lower()
                    if ltok in self._var_set:
                        dvc_to_dsgnvar[inst_canonical] = self._var_lookup[ltok]
                        break
    ...
    def refresh(self) -> None:
        ...
        - Rebuilds all derived mappings from the current in-memory netlist text.
        """
        self.dsgnvar_to_val, self.dvc_to_dsgnvar, self.dvc_to_val = self._update_circuit()
    ...
    def reload(self) -> None:
        ...
        NetlistMarkerError
            If required marker blocks are missing after reload.
        NetlistFormatError
            If netlist structural assumptions fail after reload.
        """
        self.netlist = self._load_netlist()
        self.refresh()
