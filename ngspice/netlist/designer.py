...
class Designer:
    ...
    def __init__(self, *, num_design_variables: Optional[int] = None, strict_count: bool = True) -> None:
        """
        Initialize a netlist designer with marker and count-validation policy.

        Parameters
        ----------
        num_design_variables : int or None, default=None
            Expected number of design variables to enforce during patching.
            When ``None``, count validation is disabled.
        strict_count : bool, default=True
            If ``True``, mismatched design-variable counts raise exceptions.
            If ``False``, count mismatches are tolerated.

        Raises
        ------
        ValueError
            If ``num_design_variables`` is provided and is negative.
        """
    ...
    def _refresh_circuit(circuit: Circuit) -> None:
        ...
        """
        refresh = getattr(circuit, "refresh", None)
        if callable(refresh):
            refresh()
            return

        update = getattr(circuit, "_update_circuit", None)
        if callable(update):
