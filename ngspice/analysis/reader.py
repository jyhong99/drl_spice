...
"""
Reader factory and helpers for analysis outputs.

This module exposes `create_reader` to construct a registered analysis reader
and provides thin utilities for access patterns.
"""
...
import ngspice.analysis.readers as _autoload  # noqa: F401
# NOTE:
...
def _normalize_retry_on(obj: Any) -> Tuple[Type[BaseException], ...]:
    """
    Normalize/validate a retry exception spec into a tuple of exception classes.

    Parameters
    ----------
    obj:
        Either a single exception class, or a tuple/list of exception classes.

    Returns
    -------
    tuple[type[BaseException], ...]
        Normalized tuple suitable for ``except <tuple>``.

    Raises
    ------
    ReaderNonRetryableError
        If `obj` is not a valid exception class specification.
    """
    if isinstance(obj, type) and issubclass(obj, BaseException):
        return (obj,)

    if not isinstance(obj, (tuple, list)) or len(obj) == 0:
        raise ReaderNonRetryableError(
            f"retry_on must be an exception class or non-empty tuple/list of exception classes (got {obj!r})"
        )
    ...
    for e in obj:
        if not isinstance(e, type) or not issubclass(e, BaseException):
            raise ReaderNonRetryableError(
                f"retry_on contains non-exception entry {e!r}; expected exception classes"
            )
        out.append(e)
    return tuple(out)
    ...
    def read(self, result_path: str, **kwargs: Any) -> pd.DataFrame:
        ...
        **kwargs:
        ...
        try:
            ...
            raise ReaderNonRetryableError("max_retries must be an integer") from e

        try:
            sleep_sec = float(kwargs.pop("sleep_sec", 1.0))
        except Exception as e:
            raise ReaderNonRetryableError("sleep_sec must be a float") from e

        retry_on = _normalize_retry_on(kwargs.pop("retry_on", _DEFAULT_RETRYABLE))

        if max_retries < 1:
            raise ReaderNonRetryableError(f"max_retries must be >= 1 (got {max_retries})")
        if (not math.isfinite(sleep_sec)) or sleep_sec < 0.0:
            raise ReaderNonRetryableError(f"sleep_sec must be finite and >= 0 (got {sleep_sec})")
