    ...
    def read(self, result_path: str, **kwargs: Any) -> pd.DataFrame:
        ...
        **kwargs:
        ...
        for k, (f_line, v_line) in enumerate(zip(freq_lines, val_lines)):
            ...
            # Value lines may also include a leading point index; use the last two numeric tokens.
            tokens = [p for p in _RE_SPLIT.split(v_line) if p]
            nums: list[float] = []
            for t in tokens:
                if _RE_NUM_TOKEN.fullmatch(t):
                    nums.append(float(t))
            if len(nums) < 2:
