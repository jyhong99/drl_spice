...
"""
High-level simulation orchestrator for ngspice-based analyses.

This module defines :class:`NgspiceSimulator`, which ties together:
- template rendering and patching,
- ngspice execution,
- per-analysis result readers,
- standardized ``SimResult`` production.

The implementation is intentionally defensive: most runtime failures are mapped
to structured ``SimStatus`` outcomes instead of propagating raw exceptions.
"""
...
class NgspiceSimulator:
    ...
    def __init__(
        ...
        *,
        ...
        # Pre-normalize deterministic knob baselines; per-sim runs only copy.
        self._analysis_knobs_base: Dict[str, KnobValue] = dict(
            sorted(_normalize_knobs(self.default_analysis_knobs).items(), key=lambda kv: str(kv[0]))
        )
        self._let_knobs_base: Dict[str, KnobValue] = dict(
            sorted(_normalize_knobs(self.default_let_knobs).items(), key=lambda kv: str(kv[0]))
        )
        self._merged_knobs_base: Dict[str, KnobValue] = _merge_and_sort(
            self._analysis_knobs_base, self._let_knobs_base
        )
    ...
    def _merged_knobs(self) -> Dict[str, KnobValue]:
        ...
        """
        return dict(self._merged_knobs_base)
    ...
    def simulate(
        ...
        *,
        ...
        analysis_knobs = dict(self._analysis_knobs_base)
        let_knobs = dict(self._let_knobs_base)
        merged_knobs = dict(self._merged_knobs_base)
        ...
        try:
            ...
            for kind in kinds:
                ...
                if kind not in (SPARAM_KEY, NOISE_KEY, LINEARITY_KEY):
                    return SimResult(
                        None,
                        {},
                        SimStatus.VALIDATION,
                ...
                expected_outputs = self._expected_outputs(kind=kind, knobs=merged_knobs)
                self.kernel.run(
                    ckt,
                    cwd=run_dir,
                    save_logs=True,
        ...
        except NgspiceTimeout as e:
            result = SimResult(None, {}, SimStatus.TIMEOUT, str(e), str(run_dir))

        except NgspiceExecutableNotFound as e:
            result = SimResult(None, {}, SimStatus.NGSPICE_FAIL, str(e), str(run_dir))

        except (NgspiceMissingOutputs, FileNotFoundError) as e:
            result = SimResult(None, {}, SimStatus.NO_OUTPUT, f"{type(e).__name__}: {e}", str(run_dir))

        except (NgspiceNonZeroExit, RuntimeError) as e:
            result = SimResult(None, {}, SimStatus.NGSPICE_FAIL, str(e), str(run_dir))
